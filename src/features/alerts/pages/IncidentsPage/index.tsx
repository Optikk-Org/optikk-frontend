...
export default function IncidentsPage() {
  ...
  const { data, isLoading } = useTimeRangeQuery(
    ...
    (teamId, startTime, endTime) =>
      v1Service.getIncidents(teamId, startTime, endTime, {
        statuses: statusFilter ? [statusFilter] : undefined,
        limit: PAGE_SIZE,
        offset: (page - 1) * PAGE_SIZE,
      }),
    { extraKeys: [statusFilter, page] }
  );

  const incidents = (data as any)?.incidents || [];
  const total = (data as any)?.total || 0;

  const { data: relatedAlertsRaw } = useTimeRangeQuery(
    'incident-alerts',
    (_teamId, _s, _e) => alertService.getAlertsForIncident(selectedIncident?.triggered_by),
    {
      enabled: !!selectedIncident?.triggered_by && drawerOpen,
      extraKeys: [selectedIncident?.triggered_by]
    }
  );
  const relatedAlerts = (relatedAlertsRaw as any)?.data ?? relatedAlertsRaw ?? [];

  const stats = useMemo(() => {
    const totalCount = incidents.length;
    const openCount = incidents.filter((i: any) => i.status === 'open' || i.status === 'investigating').length;

    const resolved = incidents.filter((i: any) => i.resolved_at && i.created_at);
    const mttr = resolved.length > 0
      ? resolved.reduce((sum: number, i: any) => sum + (new Date(i.resolved_at).getTime() - new Date(i.created_at).getTime()), 0) / resolved.length
      : 0;

    const acknowledged = incidents.filter((i: any) => i.acknowledged_at && i.created_at);
    const mtta = acknowledged.length > 0
      ? acknowledged.reduce((sum: number, i: any) => sum + (new Date(i.acknowledged_at).getTime() - new Date(i.created_at).getTime()), 0) / acknowledged.length
      : 0;
    ...
    <div className="incidents-page">
      ...
      <div style={{ height: boardHeight(PAGE_SIZE) }}>
        <ObservabilityDataBoard
          ...
          rows={incidents}
          rowKey={(row: any) => row.incident_id}
          entityName="incident"
            ...
            {selectedIncident.triggered_by && relatedAlerts.length > 0 && (
              <div style={{ marginTop: 24 }}>
                ...
                {relatedAlerts.map((alert: any) => {
                  ...
                  const sColor = ALERT_STATUSES.find((s) => s.value === alert.status?.toUpperCase())?.color || '#98A2B3';
                  return (
                    <div key={alert.id} style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '6px 0', borderBottom: '1px solid var(--border-color)' }}>
                      ...
                      <Tag color={sColor} style={{ color: '#fff' }}>{alert.status?.toUpperCase()}</Tag>
