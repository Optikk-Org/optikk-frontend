...
const normalizeTimeSeriesPoint = (p = {}) => ({
  ...p,
...
const normalizeEndpointMetric = (m = {}) => ({
  ...m,
...
function KpiCard({ title, value, icon: Icon, accentColor, accentBg, trend }) {
  return (
    <div
      ...
      style={{ '--kpi-accent': accentColor, '--kpi-accent-from': accentColor + '33', '--kpi-accent-bg': accentBg }}
    >
...
export default function TracesPage() {
  ...
  const { data: endpointTimeseriesRaw } = useTimeRangeQuery('endpoints-timeseries-traces', (t, s, e) => v1Service.getEndpointTimeSeries(t, s, e));
  const { data: endpointMetricsRaw } = useTimeRangeQuery('endpoints-metrics-traces', (t, s, e) => v1Service.getEndpointMetrics(t, s, e));
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: () => {
      ...
      return v1Service.getTraces(selectedTeamId, startTime, endTime, backendParams);
    },
  ...
  return (
    <div className="traces-page">
      ...
      <div className="traces-kpi-row">
        <KpiCard title="Total Traces" value={formatNumber(totalTraces || 0)} icon={Activity} accentColor="#5E60CE" accentBg="rgba(94,96,206,0.12)" />
        <KpiCard title="Error Rate" value={`${(errorRate || 0).toFixed(2)}%`} icon={AlertCircle} accentColor={errorRate > 5 ? '#F04438' : '#73C991'} accentBg={errorRate > 5 ? 'rgba(240,68,56,0.12)' : 'rgba(115,201,145,0.12)'} />
        <KpiCard title="P95 Latency" value={formatDuration(p95 || 0)} icon={Zap} accentColor="#10B981" accentBg="rgba(16,185,129,0.12)" />
        <KpiCard title="P99 Latency" value={formatDuration(p99 || 0)} icon={Clock} accentColor="#F59E0B" accentBg="rgba(245,158,11,0.12)" />
      </div>
