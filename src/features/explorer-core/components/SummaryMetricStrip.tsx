  ...
  return (
    <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
      {metrics.map((metric) => (
        ...
        >
          <div className="flex items-center justify-between text-[11px] uppercase tracking-[0.12em] text-[var(--text-muted)]">
            <span>{metric.label}</span>
            <span className="rounded-full border border-current/10 bg-black/10 p-1">
              {iconForTone(metric.tone)}
            </span>
          </div>
          <div className="mt-4 text-[28px] font-semibold leading-none text-[var(--text-primary)]">
            {metric.value}
          </div>
          {metric.hint ? (
            <p className="mt-3 text-xs leading-5 text-[var(--text-secondary)]">{metric.hint}</p>
          ) : null}
        </Card>
      ))}
    </div>
