...
function structuredFiltersToQueryString(
...
/** Merge structured filters and optional logs “errors only” into the backend query string. */
export function buildLogsQueryString(params: {
  filters: StructuredFilter[];
  errorsOnly: boolean;
}): string {
  const parts: string[] = [];
...
/** Merge structured filters, errors-only, and selected service for traces hub query string. */
export function buildTracesQueryString(params: {
  filters: StructuredFilter[];
  errorsOnly: boolean;
