...
import type { AITrendBucket } from "../types";
...
function VolumeBar({ bucket, maxTotal }: VolumeBarProps) {
  ...
  const tooltip = (
    <div className="flex flex-col gap-1">
      ...
      {errorCount > 0 && <span className="text-[var(--color-error)]">Errors: {formatNumber(errorCount)}</span>}
      {okCount > 0 && <span className="text-[var(--color-success)]">OK: {formatNumber(okCount)}</span>}
      <span className="text-[var(--text-muted)]">Avg latency: {bucket.avg_latency_ms.toFixed(0)}ms</span>
      {bucket.total_tokens > 0 && (
        <span className="text-[var(--text-muted)]">Tokens: {formatNumber(bucket.total_tokens)}</span>
      )}
    </div>
  );
  ...
  return (
    <Tooltip content={tooltip} placement="top">
      <div className="relative flex flex-1 cursor-crosshair items-end justify-center" style={{ minWidth: 2 }}>
        {totalCount > 0 && (
          <div
          ...
          >
            ...
            {okCount > 0 && (
              <div style={{ flex: okCount, background: STATUS_COLORS.ok }} />
            )}
          </div>
        )}
      </div>
...
interface AIVolumeChartProps {
  buckets: AITrendBucket[];
  isLoading: boolean;
}

export default function AIVolumeChart({ buckets, isLoading }: AIVolumeChartProps) {
  const maxTotal = useMemo(
    () => Math.max(...buckets.map((b) => b.total_calls), 1),
    [buckets]
  );
    ...
    <div className="space-y-1">
      ...
      <div className="flex items-center gap-4 text-[11px] text-[var(--text-muted)]">
        <div className="flex items-center gap-1.5">
          <span className="inline-block h-2 w-2 rounded-full" style={{ background: STATUS_COLORS.ok }} />
          OK
        </div>
        <div className="flex items-center gap-1.5">
          <span className="inline-block h-2 w-2 rounded-full" style={{ background: STATUS_COLORS.errors }} />
          Error
        </div>
      </div>
    </div>
