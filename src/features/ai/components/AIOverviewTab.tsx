import { AlertTriangle, ArrowUpRight, BarChart3, Coins, DollarSign, Gauge, Sparkles, Zap } from "lucide-react";
...
import type { AICallRecord, AIExplorerFacets, AISummary, AITrendBucket } from "../types";
import { estimateCost, formatCost } from "../utils/costCalculator";
...
function SummaryTile({ icon, label, value, subValue, tone = "default" }: SummaryTileProps) {
  return (
    <Card padding="lg" className={cn("border shadow-sm", toneClasses[tone])}>
      ...
      {subValue && (
        <div className="mt-1.5 text-[12px] text-[var(--text-muted)]">{subValue}</div>
      )}
    </Card>
  );
}
...
function ProviderBreakdown({ facets }: { facets: AIExplorerFacets }) {
...
function TopErrorsTable({ calls }: { calls: AICallRecord[] }) {
  const errorCalls = useMemo(
    () =>
      calls
        .filter((c) => c.status === "ERROR" || c.status === "error")
        .slice(0, 5),
    [calls]
  );
  ...
  return (
    <div className="space-y-2">
      {errorCalls.map((call) => (
        <div
          key={`${call.trace_id}-${call.span_id}`}
        ...
        >
          ...
          <span className="min-w-0 flex-1 truncate font-mono text-[11px] text-[var(--text-primary)]">
            {call.ai_request_model}
          </span>
          <span className="text-[11px] text-[var(--text-muted)]">
            {call.error_type || call.status_message || "Unknown error"}
          </span>
          <span className="text-[11px] text-[var(--text-muted)]">
...
export default function AIOverviewTab({
  ...
  const errorRate =
    summary.total_calls > 0
      ? (summary.error_calls * 100) / summary.total_calls
      : 0;

  const estimatedTotalCost = useMemo(
    () => calls.reduce((sum, c) => sum + c.estimated_cost, 0),
    [calls]
  );
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Two-column: Model breakdown + Provider breakdown */}
      <div className="grid gap-4 lg:grid-cols-2">
        <Card padding="lg" className="border border-[var(--border-color)]">
          <div className="mb-4 flex items-center gap-2 text-[13px] font-medium text-[var(--text-primary)]">
