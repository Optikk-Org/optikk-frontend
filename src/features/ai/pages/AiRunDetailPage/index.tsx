        ...
        {isLoading ? (
          ...
          <div className="p-6">
            <div className="px-4 py-3 rounded-lg bg-[var(--error-bg,rgba(240,68,56,0.08))] border border-[var(--error-border,rgba(240,68,56,0.2))] text-[var(--error-text,#f04438)]">
              ...
              <div className="mt-1 text-[13px]">{getErrorMessage(detailError, 'The backend request failed for this run.')}</div>
            </div>
          </div>
        ) : (
          <div className="p-10 text-center text-[var(--text-muted)]">
    ...
    <div className="max-w-[1200px] mx-auto px-0 pb-6">
      ...
      <div className="grid grid-cols-3 gap-4 mb-5">
        ...
        <div className="bg-[var(--glass-bg)] border border-[var(--border-color)] rounded-[10px] p-4">
          <h3 className="text-[12px] font-semibold text-[var(--text-muted)] uppercase tracking-[0.5px] mt-0 mb-3">Metadata</h3>
        ...
        {/* Performance card */}
        <div className="bg-[var(--glass-bg)] border border-[var(--border-color)] rounded-[10px] p-4">
          <h3 className="text-[12px] font-semibold text-[var(--text-muted)] uppercase tracking-[0.5px] mt-0 mb-3">Performance</h3>
        ...
        {/* Tokens card */}
        <div className="bg-[var(--glass-bg)] border border-[var(--border-color)] rounded-[10px] p-4">
          <h3 className="text-[12px] font-semibold text-[var(--text-muted)] uppercase tracking-[0.5px] mt-0 mb-3">Tokens</h3>
          ...
          <div className="flex h-5 rounded overflow-hidden mt-2">
            <div className="bg-[#3b82f6]" style={{ width: `${inputPct}%` }} title={`Input: ${formatNumber(detail.inputTokens)}`} />
            <div className="bg-[#10b981]" style={{ width: `${100 - inputPct}%` }} title={`Output: ${formatNumber(detail.outputTokens)}`} />
          </div>
      ...
      <div className="bg-[var(--glass-bg)] border border-[var(--border-color)] rounded-[10px] mb-5 overflow-hidden">
        ...
        {messagesError && (
          <div className="px-[18px] pt-4">
            <div className="px-4 py-3 rounded-lg bg-[var(--error-bg,rgba(240,68,56,0.08))] border border-[var(--error-border,rgba(240,68,56,0.2))] text-[var(--error-text,#f04438)]">
              ...
              <div className="mt-1 text-[13px]">{getErrorMessage(messagesError, 'The backend request for run messages failed.')}</div>
            </div>
          </div>
        )}
        ...
        {messages.map((msg: LLMMessage, i: number) => (
          <div key={i} className="px-[18px] py-[14px] border-b border-[var(--border-color)] last:border-b-0">
            <div className={cn(
        ...
        <div className="bg-[var(--glass-bg)] border border-[var(--border-color)] rounded-[10px] overflow-hidden">
          ...
          <div className="px-[18px] py-4">
            <div className="px-4 py-3 rounded-lg bg-[var(--error-bg,rgba(240,68,56,0.08))] border border-[var(--error-border,rgba(240,68,56,0.2))] text-[var(--error-text,#f04438)]">
              ...
              <div className="mt-1 text-[13px]">{getErrorMessage(contextError, 'The backend request for execution context failed.')}</div>
            </div>
          </div>
        </div>
      )}
