...
function pct(v: any, decimals = 1) {
  const num = n(v);
  return num == null ? 'N/A' : `${num.toFixed(decimals)}%`;
}

function dollar(v: any, decimals = 4) {
  const num = n(v);
  return num == null ? 'N/A' : `$${num.toFixed(decimals)}`;
}

function naSpan(v?: any) {
...
const latColor = (ms: any) => ms > 5000 ? '#F04438' : ms > 2000 ? '#F79009' : '#73C991';
const rateColor = (r: any) => r > 5 ? '#F04438' : r > 1 ? '#F79009' : '#73C991';

// ─── OVERVIEW TAB ────────────────────────────────────────────────────────────

function OverviewTab({ summary, summaryLoading, config, dataSources, selectedModel }: any) {
  const s = summary || {};
...
function PerformanceTab({ perfMetrics, metricsLoading, config, dataSources, selectedModel }: any) {
  const tableColumns = [
    { title: 'Model', dataIndex: 'model_name', key: 'model_name', render: (v: any) => <Tag className="ai-model-tag">{v || 'unknown'}</Tag> },
    { title: 'Provider', dataIndex: 'model_provider', key: 'model_provider', render: (v: any) => v ? <span style={{ color: 'var(--text-muted)', fontSize: 12 }}>{v}</span> : naSpan() },
    { title: 'Type', dataIndex: 'request_type', key: 'request_type', render: (v: any) => v ? <Tag style={{ fontSize: 11 }}>{v}</Tag> : naSpan() },
    { title: 'Requests', dataIndex: 'total_requests', key: 'total_requests', render: (v: any) => formatNumber(Number(v)), sorter: (a: any, b: any) => Number(a.total_requests) - Number(b.total_requests), align: 'right' as const },
    { title: 'QPS', dataIndex: 'avg_qps', key: 'avg_qps', render: (v: any) => { const x = n(v); return x == null ? naSpan() : <span style={{ fontWeight: 600 }}>{x.toFixed(3)}</span>; }, sorter: (a: any, b: any) => (n(a.avg_qps) ?? -1) - (n(b.avg_qps) ?? -1), align: 'right' as const },
    { title: 'Avg Latency', dataIndex: 'avg_latency_ms', key: 'avg_latency_ms', render: (v: any) => { const x = n(v); return x == null ? naSpan() : <span style={{ color: latColor(x), fontWeight: 600 }}>{formatDuration(x)}</span>; }, sorter: (a: any, b: any) => (n(a.avg_latency_ms) ?? -1) - (n(b.avg_latency_ms) ?? -1), align: 'right' as const },
    { title: 'P50', dataIndex: 'p50_latency_ms', key: 'p50_latency_ms', render: (v: any) => { const x = n(v); return x == null ? naSpan() : formatDuration(x); }, sorter: (a: any, b: any) => (n(a.p50_latency_ms) ?? -1) - (n(b.p50_latency_ms) ?? -1), align: 'right' as const },
    { title: 'P95', dataIndex: 'p95_latency_ms', key: 'p95_latency_ms', render: (v: any) => { const x = n(v); return x == null ? naSpan() : <span style={{ color: latColor(x), fontWeight: 600 }}>{formatDuration(x)}</span>; }, sorter: (a: any, b: any) => (n(a.p95_latency_ms) ?? -1) - (n(b.p95_latency_ms) ?? -1), align: 'right' as const },
    { title: 'P99', dataIndex: 'p99_latency_ms', key: 'p99_latency_ms', render: (v: any) => { const x = n(v); return x == null ? naSpan() : <span style={{ color: latColor(x * 1.2) }}>{formatDuration(x)}</span>; }, sorter: (a: any, b: any) => (n(a.p99_latency_ms) ?? -1) - (n(b.p99_latency_ms) ?? -1), align: 'right' as const },
    { title: 'Tokens/sec', dataIndex: 'avg_tokens_per_sec', key: 'avg_tokens_per_sec', render: (v: any) => { const x = n(v); return x == null ? naSpan() : <span style={{ fontWeight: 600, color: '#9E77ED' }}>{x.toFixed(1)}</span>; }, sorter: (a: any, b: any) => (n(a.avg_tokens_per_sec) ?? -1) - (n(b.avg_tokens_per_sec) ?? -1), align: 'right' as const },
    { title: 'Timeouts', dataIndex: 'timeout_count', key: 'timeout_count', render: (v: any) => { const x = n(v) ?? 0; return <span style={{ color: x > 0 ? '#F04438' : '#73C991', fontWeight: 600 }}>{formatNumber(x)}</span>; }, sorter: (a: any, b: any) => (n(a.timeout_count) ?? 0) - (n(b.timeout_count) ?? 0), align: 'right' as const },
    { title: 'Error Rate', dataIndex: 'error_rate', key: 'error_rate', render: (v: any) => { const x = n(v); return x == null ? naSpan() : <span style={{ color: rateColor(x), fontWeight: 600 }}>{x.toFixed(2)}%</span>; }, sorter: (a: any, b: any) => (n(a.error_rate) ?? -1) - (n(b.error_rate) ?? -1), align: 'right' as const },
  ];

  const data = useMemo(() => {
    const raw = Array.isArray(perfMetrics) ? perfMetrics : [];
    return selectedModel ? raw.filter((r: any) => r.model_name === selectedModel) : raw;
    ...
    <>
      ...
      <Card title="Per-Model Performance" className="ai-chart-card" style={{ marginTop: 16 }}>
        {metricsLoading ? <Skeleton active paragraph={{ rows: 6 }} /> : data.length === 0 ? <Empty description="No data" /> : (
          <Table dataSource={data.map((r: any, i: number) => ({ ...r, key: i }))} columns={tableColumns as any} size="small" pagination={{ pageSize: 20 }} scroll={{ x: 1600 }} />
        )}
      </Card>
    </>
  );
...
function CostTab({ costMetrics, costLoading, config, dataSources, selectedModel }: any) {
  const data = useMemo(() => {
  ...
  const tableColumns = [
    ...
    { title: 'Provider', dataIndex: 'model_provider', key: 'model_provider', render: (v: any) => v ? <span style={{ color: 'var(--text-muted)', fontSize: 12 }}>{v}</span> : naSpan() },
    { title: 'Requests', dataIndex: 'total_requests', key: 'total_requests', render: (v: any) => formatNumber(Number(v)), sorter: (a: any, b: any) => Number(a.total_requests) - Number(b.total_requests), align: 'right' as const },
    { title: 'Total Cost', dataIndex: 'total_cost_usd', key: 'total_cost_usd', render: (v: any) => { const x = n(v); return x == null ? naSpan() : <span style={{ color: '#F79009', fontWeight: 700 }}>{dollar(x)}</span>; }, sorter: (a: any, b: any) => (n(a.total_cost_usd) ?? -1) - (n(b.total_cost_usd) ?? -1), align: 'right' as const },
    { title: 'Cost / Query', dataIndex: 'avg_cost_per_query', key: 'avg_cost_per_query', render: (v: any) => { const x = n(v); return x == null ? naSpan() : <span style={{ color: '#FDB022', fontWeight: 600 }}>{dollar(x, 5)}</span>; }, sorter: (a: any, b: any) => (n(a.avg_cost_per_query) ?? -1) - (n(b.avg_cost_per_query) ?? -1), align: 'right' as const },
    { title: 'Prompt Tokens', dataIndex: 'total_prompt_tokens', key: 'total_prompt_tokens', render: (v: any) => { const x = n(v); return x == null ? naSpan() : formatNumber(x); }, sorter: (a: any, b: any) => (n(a.total_prompt_tokens) ?? -1) - (n(b.total_prompt_tokens) ?? -1), align: 'right' as const },
    { title: 'Completion Tokens', dataIndex: 'total_completion_tokens', key: 'total_completion_tokens', render: (v: any) => { const x = n(v); return x == null ? naSpan() : formatNumber(x); }, sorter: (a: any, b: any) => (n(a.total_completion_tokens) ?? -1) - (n(b.total_completion_tokens) ?? -1), align: 'right' as const },
    { title: 'Total Tokens', dataIndex: 'total_tokens', key: 'total_tokens', render: (v: any) => { const x = n(v); return x == null ? naSpan() : <span style={{ fontWeight: 600, color: '#9E77ED' }}>{formatNumber(x)}</span>; }, sorter: (a: any, b: any) => (n(a.total_tokens) ?? -1) - (n(b.total_tokens) ?? -1), align: 'right' as const },
    { title: 'Cache Hit %', dataIndex: 'cache_hit_rate', key: 'cache_hit_rate', render: (v: any) => { const x = n(v); if (x == null) return naSpan(); const color = x > 70 ? '#73C991' : x > 30 ? '#F79009' : '#F04438'; return <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}><Progress percent={Math.min(x, 100)} size="small" showInfo={false} strokeColor={color} style={{ width: 60 }} /><span style={{ color, fontWeight: 600, fontSize: 12 }}>{x.toFixed(1)}%</span></div>; }, sorter: (a: any, b: any) => (n(a.cache_hit_rate) ?? -1) - (n(b.cache_hit_rate) ?? -1) },
  ];
    ...
    <>
      ...
      <Card title="Per-Model Cost Breakdown" className="ai-chart-card" style={{ marginTop: 16 }}>
        {costLoading ? <Skeleton active paragraph={{ rows: 6 }} /> : data.length === 0 ? <Empty description="No data" /> : (
          <Table dataSource={data.map((r: any, i: number) => ({ ...r, key: i }))} columns={tableColumns as any} size="small" pagination={{ pageSize: 20 }} scroll={{ x: 1400 }} />
        )}
      </Card>
    </>
  );
...
function SecurityTab({ secMetrics, secLoading, config, dataSources, selectedModel }: any) {
  const data = useMemo(() => {
  ...
  const tableColumns = [
    { title: 'Model', dataIndex: 'model_name', key: 'model_name', render: (v: any) => <Tag className="ai-model-tag">{v || 'unknown'}</Tag> },
    { title: 'Requests', dataIndex: 'total_requests', key: 'total_requests', render: (v: any) => formatNumber(Number(v)), align: 'right' as const },
    { title: 'PII Detected', dataIndex: 'pii_detected_count', key: 'pii_detected_count', render: (v: any) => { const x = n(v) ?? 0; return <Tooltip title={`${x} requests with PII`}><span style={{ color: x > 0 ? '#F04438' : '#73C991', fontWeight: 600 }}>{formatNumber(x)}</span></Tooltip>; }, align: 'right' as const },
    { title: 'PII Rate', dataIndex: 'pii_detection_rate', key: 'pii_detection_rate', render: (v: any) => { const x = n(v); if (x == null) return naSpan(); const color = rateColor(x); return <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}><Progress percent={Math.min(x, 100)} size="small" showInfo={false} strokeColor={color} style={{ width: 60 }} /><span style={{ color, fontWeight: 600, fontSize: 12 }}>{x.toFixed(2)}%</span></div>; } },
    { title: 'Guardrail Blocks', dataIndex: 'guardrail_blocked_count', key: 'guardrail_blocked_count', render: (v: any) => { const x = n(v) ?? 0; return <span style={{ color: x > 0 ? '#F79009' : '#73C991', fontWeight: 600 }}>{formatNumber(x)}</span>; }, align: 'right' as const },
    { title: 'Block Rate', dataIndex: 'guardrail_block_rate', key: 'guardrail_block_rate', render: (v: any) => { const x = n(v); return x == null ? naSpan() : <span style={{ color: rateColor(x), fontWeight: 600 }}>{x.toFixed(2)}%</span>; }, align: 'right' as const },
    { title: 'Overall Safety', key: 'safety', render: (_: any, r: any) => { const piiRate = n(r.pii_detection_rate) ?? 0; const blockRate = n(r.guardrail_block_rate) ?? 0; const ok = piiRate < 1 && blockRate < 1; return <Tooltip title={`PII ${piiRate.toFixed(2)}%, Blocks ${blockRate.toFixed(2)}%`}><Badge status={ok ? 'success' : 'error'} text={ok ? 'Healthy' : 'Attention'} /></Tooltip>; } },
  ];
    ...
    <>
      ...
      <Card title="Per-Model Security" className="ai-chart-card" style={{ marginTop: 16 }}>
        {secLoading ? <Skeleton active paragraph={{ rows: 6 }} /> : data.length === 0 ? <Empty description="No data" /> : (
          <Table dataSource={data.map((r: any, i: number) => ({ ...r, key: i }))} columns={tableColumns as any} size="small" pagination={{ pageSize: 20 }} scroll={{ x: 1000 }} />
        )}
      </Card>
    </>
  );
