...
import type { MetricQueryDefinition, TimeStep } from '../types';
...
export function useMetricsExplorerQuery(queries: MetricQueryDefinition[], step: TimeStep) {
  ...
  return useQuery({
    queryKey: ['metrics', 'explorer', selectedTeamId, queriesHash, startTime, endTime, step, refreshKey],
    queryFn: () => metricsExplorerApi.query(buildQueryRequest(queries, startTime, endTime, step)),
