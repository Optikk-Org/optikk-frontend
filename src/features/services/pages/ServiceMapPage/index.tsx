...
export default function ServiceMapPage() {
  ...
  const allEdges: any[] = (topologyData as any)?.edges || [];

  // Filter nodes by health status
  const filteredNodes = useMemo(() => {
    if (healthFilter === 'all') return allNodes;
    return allNodes.filter((n: any) => n.status?.toLowerCase() === healthFilter);
  ...
  const filteredEdges = useMemo(() => {
    ...
    return allEdges.filter((e: any) => nodeNames.has(e.source) && nodeNames.has(e.target));
  }, [allEdges, filteredNodes]);

  const stats = useMemo(() => {
    const totalServices = allNodes.length;
    const healthyServices = allNodes.filter((n: any) => n.status?.toLowerCase() === 'healthy').length;
    const totalDependencies = allEdges.length;
    return { totalServices, healthyServices, totalDependencies };
  }, [allNodes, allEdges]);

  const handleNodeClick = (node: any) => navigate(`/services/${node.name}`);
  const handleServiceClick = (serviceName: string) => navigate(`/services/${serviceName}`);

  const healthOptions = [
    { key: 'all', label: 'All', count: allNodes.length },
    { key: 'healthy', label: 'Healthy', count: allNodes.filter((n: any) => n.status?.toLowerCase() === 'healthy').length, color: '#73C991' },
    { key: 'degraded', label: 'Degraded', count: allNodes.filter((n: any) => n.status?.toLowerCase() === 'degraded').length, color: '#F79009' },
    { key: 'unhealthy', label: 'Unhealthy', count: allNodes.filter((n: any) => n.status?.toLowerCase() === 'unhealthy').length, color: '#F04438' },
  ];
                  ...
                  <List.Item className="service-list-item" onClick={() => handleServiceClick(node.name)}>
                    <div className="service-list-item-content">
                      <div className="service-info">
                        <HealthIndicator status={node.status} size={8} />
