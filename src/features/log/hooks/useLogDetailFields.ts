...
export function useLogDetailFields(log: LogEntry | null) {
  return useMemo(() => {
    ...
    return [
      ...
      { key: 'service_name', label: 'Service', value: log['serviceName'] ?? log['service_name'], filterable: true },
      ...
      { key: 'logger', label: 'Logger', value: log['logger'] },
      { key: 'thread', label: 'Thread', value: log['thread'] },
      { key: 'trace_id', label: 'Trace ID', value: log['traceId'] ?? log['trace_id'], filterable: true },
      { key: 'span_id', label: 'Span ID', value: log['spanId'] ?? log['span_id'] },
