...
export default function LogRow({
  ...
  const renderCell = (colKey: string): ReactNode => {
    ...
    switch (colKey) {
      case 'timestamp':
        return <span className="log-cell log-cell--mono log-cell--secondary">{tsLabel(log.timestamp)}</span>;
      case 'level':
      ...
      case 'host':
        return <span className="log-cell log-cell--mono log-cell--secondary">{toDisplayText(log.host || log.pod)}</span>;
      case 'logger':
        return <span className="log-cell log-cell--mono log-cell--muted truncate">{toDisplayText(log.logger)}</span>;
      case 'trace_id':
        return <span className="log-cell log-cell--mono log-cell--muted truncate">{toDisplayText(log.traceId || log.trace_id)}</span>;
      case 'thread':
        return <span className="log-cell log-cell--mono log-cell--muted">{toDisplayText(log.thread)}</span>;
      case 'container':
