...
export const calculateTrends = (
  ...
  const recentAvg = recent.reduce((sum: number, metric) => sum + metric.request_count, 0) / recent.length;
  const olderAvg = older.reduce((sum: number, metric) => sum + metric.request_count, 0) / older.length;
  const requestTrend = olderAvg > 0 ? ((recentAvg - olderAvg) / olderAvg) * 100 : 0;

  const recentErrorRate = recent.reduce((sum: number, metric) => {
    const total = metric.request_count;
    const errors = metric.error_count;
    return sum + (total > 0 ? (errors / total) * 100 : 0);
  }, 0) / recent.length;

  const olderErrorRate = older.reduce((sum: number, metric) => {
    const total = metric.request_count;
    const errors = metric.error_count;
    return sum + (total > 0 ? (errors / total) * 100 : 0);
  }, 0) / older.length;
