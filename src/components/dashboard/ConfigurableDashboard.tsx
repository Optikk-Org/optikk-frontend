...
function LatencyHistogramRenderer({ chartConfig, dataSources, extraContext }: { chartConfig: any, dataSources: any, extraContext?: any }) {
  const rawData = dataSources?.[chartConfig.dataSource];
  const traces = useMemo(() => {
    const arr = Array.isArray(rawData) ? rawData : [];
    if (arr.length > 0 && (arr[0].duration_ms != null || arr[0].durationMs != null)) return arr;
    const bucketMidpoint = (b: string): number => ({
...
function LatencyHeatmapRenderer({ chartConfig, dataSources, extraContext }: { chartConfig: any, dataSources: any, extraContext?: any }) {
  const rawData = dataSources?.[chartConfig.dataSource];
  const data = useMemo(() => {
    const key = chartConfig.dataKey;
    const arr = key ? rawData?.[key] : rawData;
...
function AiLineRenderer({ chartConfig, dataSources, extraContext }: { chartConfig: any, dataSources: any, extraContext: any }) {
  ...
  const tickCallback = chartConfig.yPrefix
    ? (v: any) => `${chartConfig.yPrefix}${Number(v).toFixed(chartConfig.yDecimals ?? 2)}`
    : undefined;
...
function AiBarRenderer({ chartConfig, dataSources, extraContext }: { chartConfig: any, dataSources: any, extraContext: any }) {
  ...
  const chartData = useMemo(() => {
    ...
    if (chartConfig.valueKeys && chartConfig.valueKeys.length > 0) {
      ...
      const datasets = chartConfig.valueKeys.map((vk: string, i: number) => ({
    ...
    if (chartConfig.bucketKey) {
      ...
      const allBuckets = Array.from(new Set(filtered.map((r) => r[chartConfig.bucketKey]))).sort((a: any, b: any) => a - b);
      const labels = allBuckets.map((b) => `${b}ms`);
      const datasets = Object.keys(groups).map((g, i) =>
        createBarDataset(g, allBuckets.map((b) => groups[g][b as any] ?? 0), getChartColor(i))
      );
      return { labels, datasets, hasData: datasets.length > 0 };
    }
  ...
  const height = chartConfig.height || 220;
  const tickCallback = chartConfig.yPrefix
    ? (v: any) => `${chartConfig.yPrefix}${Number(v).toFixed(chartConfig.yDecimals ?? 4)}`
    : undefined;
...
const SPECIALIZED_RENDERERS: Record<string, any> = {
...
function buildQueueEndpoints(topQueues: any[], sortField: string, scope: string) {
  ...
  const queueSeriesKey = (q: any) => `${strValue(q, ['queue_name', 'queueName'], 'unknown')}::${strValue(q, ['service_name', 'serviceName'], 'unknown')}`;
  return [...topQueues]
 ...
 */
function ConfigurableChartCard({ chartConfig, dataSources, extraContext }: { chartConfig: any, dataSources: any, extraContext: any }) {
  ...
  // Check for specialized renderers (including log-histogram)
 ...
 */
export default function ConfigurableDashboard({ config, dataSources = {}, isLoading = false, extraContext = {} }: { config: any, dataSources?: any, isLoading?: boolean, extraContext?: any }) {
