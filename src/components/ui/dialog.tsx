    ...
    <DialogPrimitive.Root open={open} onOpenChange={(v) => { if (!v && closable) onClose(); }}>
      <DialogPrimitive.Portal>
        ...
        <DialogPrimitive.Content
          className={cn(
            'fixed left-1/2 top-[15vh] z-50 -translate-x-1/2 rounded-xl border border-[var(--border-color)] bg-[var(--bg-secondary)] shadow-lg focus:outline-none data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=open]:fade-in-0 data-[state=closed]:zoom-out-95 data-[state=open]:zoom-in-95 data-[state=closed]:slide-out-to-left-1/2 data-[state=open]:slide-in-from-left-1/2',
            className,
          )}
        ...
        >
          {title || closable ? (
            <div className="flex items-center justify-between border-b border-[var(--border-color)] px-5 py-3.5">
              ...
              {closable ? (
                <DialogPrimitive.Close className="ml-auto flex h-6 w-6 items-center justify-center rounded text-[var(--text-muted)] transition-colors hover:text-[var(--text-primary)]">
                  <X size={16} />
