...
export interface SimpleTableColumn<RowType extends AnyRow = AnyRow> {
...
export interface SimpleTableProps<RowType extends AnyRow = AnyRow> {
...
type SortableRow<RowType extends AnyRow> = Row<RowType>;

function createSortingFn<RowType extends AnyRow>(
...
function toColumnId<RowType extends AnyRow>(
  column: SimpleTableColumn<RowType>,
): string {
  return String(column.key ?? column.dataIndex ?? '');
}

function SimpleTable<RowType extends AnyRow = AnyRow>({
      ...
      incomingColumns.map((column) => {
        ...
        return {
          ...
          accessorFn: (row: RowType) =>
            column.dataIndex ? row[String(column.dataIndex)] : undefined,
          header: () => column.title,
          ...
          cell: (info) => {
            const value = column.dataIndex
              ? info.row.original[String(column.dataIndex)]
              : undefined;
  ...
  const getRowId = useMemo(() => {
    ...
    return (row: RowType, index: number) => String(row.key ?? index);
  }, [rowKey]);
  ...
  return (
    <div className={cn('w-full overflow-auto', className)}>
      <div style={scroll?.y ? { maxHeight: scroll.y, overflowY: 'auto' } : undefined}>
        <Table
          className={cn('border-collapse table-fixed', sizeClasses[size])}
                ...
                {headerGroup.headers.map((header) => {
                  ...
                  return (
                    <TableHead
                      key={header.id}
                      className={cn(
                        'relative border-b border-[var(--glass-border)] bg-[rgba(255,255,255,0.03)] font-semibold normal-case tracking-normal',
                ...
                <TableRow key={row.id} className={rowCls} {...rowProps}>
                  {row.getVisibleCells().map((cell) => {
                    ...
                    return (
                      <TableCell
                        key={cell.id}
                        className={cn(meta?.ellipsis && 'max-w-0 truncate')}
                        style={{
                          textAlign: meta?.align ?? 'left',
                          width: resolvedWidth,
                        }}
      ...
      {pagination && table.getPageCount() > 1 ? (
        <div className="flex items-center justify-between px-3 py-2 text-[12px] text-[var(--text-secondary)]">
          ...
          <div className="flex items-center gap-2">
            {typeof pagination === 'object' && pagination.showSizeChanger ? (
              <select
                ...
                className="h-6 rounded border border-[var(--border-color)] bg-[var(--bg-tertiary)] px-1 text-[11px] text-[var(--text-primary)]"
              >
            ...
            <button
              ...
              className="rounded border border-[var(--border-color)] bg-[var(--bg-tertiary)] px-2 py-0.5 disabled:opacity-40"
            >
              ‹
            </button>
            <span>
