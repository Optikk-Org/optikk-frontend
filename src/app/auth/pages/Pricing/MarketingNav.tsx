  ...
  return (
    <>
      <nav style={{
        ...
        padding: '0 48px', transition: 'all 0.3s ease',
        ...
        <div 
          onClick={() => navigate('/')}
          style={{ display: 'flex', alignItems: 'center', gap: 10, cursor: 'pointer', fontFamily: "'DM Serif Display', serif", fontSize: 22, color: '#F8FAFC', letterSpacing: '0.5px' }}
        >
          <BrandIcon />
          Optikk
        </div>
        ...
        <div className="desktop-links" style={{ display: 'flex', alignItems: 'center', gap: 32 }}>
          {links.map(lx => (
            lx.ext ? (
              <a key={lx.label} href={lx.path} target="_blank" rel="noreferrer" className="nav-link">{lx.label}</a>
            ) : (
              <div key={lx.label} className="nav-link" onClick={() => navigate(lx.path)}>
                {lx.label}
                {location.pathname === lx.path && (
                  <motion.div layoutId="nav-indicator" style={{ position: 'absolute', bottom: -6, left: 0, right: 0, height: 2, background: '#22D3EE', borderRadius: 2 }} />
                )}
              </div>
            )
          ))}
        ...
        <div className="desktop-links" style={{ display: 'flex', gap: 12, alignItems: 'center' }}>
          <button className="nav-cta-btn" onClick={() => navigate('/login')}>Get Started</button>
        </div>
        ...
        <button
          ...
          style={{ background: 'transparent', border: 'none', color: '#F8FAFC', cursor: 'pointer', padding: 4 }}
        >
        ...
        {mobileMenuOpen && (
          ...
          >
            {links.map(lx => (
              <div
                key={lx.label}
                style={{ color: '#F8FAFC', fontSize: 18, fontWeight: 500, fontFamily: "'Inter', sans-serif" }}
                onClick={() => {
