...
function formatLocal(date: Date, fmt: 'datetime' | 'date' | 'time' | 'iso'): string {
  switch (fmt) {
    case 'date': return dateFnsFormat(date, 'yyyy-MM-dd');
    case 'time': return dateFnsFormat(date, 'HH:mm:ss');
    case 'iso': return dateFnsFormat(date, "yyyy-MM-dd'T'HH:mm:ss");
    case 'datetime':
    default: return dateFnsFormat(date, 'yyyy-MM-dd HH:mm');
  }
}
