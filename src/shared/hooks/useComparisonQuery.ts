...
export function useComparisonQuery<TData = unknown>(
  ...
  const comparisonBounds = computeComparisonBounds(comparisonMode, Number(startTime), Number(endTime));

  const comparisonQuery = useQuery<TData, Error>({
    queryKey: [key, 'comparison', selectedTeamId, rangeKey, comparisonMode, refreshKey, ...extraKeys],
    queryFn: async () => {
