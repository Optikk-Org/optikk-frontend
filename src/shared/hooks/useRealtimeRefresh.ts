...
export function useRealtimeRefresh(options: UseRealtimeRefreshOptions = {}) {
  ...
  const invalidateForSignal = useCallback(
    (signal: SignalType) => {
      const keys = SIGNAL_QUERY_KEYS[signal];
      if (!keys) return;
  ...
  useEffect(() => {
    ...
    const removeListener = addEventListener('data-update', (data: unknown) => {
      const event = data as DataUpdateEvent;
      if (event?.signal) {
        invalidateForSignal(event.signal);
      }
    });
