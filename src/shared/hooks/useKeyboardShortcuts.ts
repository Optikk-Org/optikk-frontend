  ...
  useEffect(() => {
    const handler = (e: KeyboardEvent) => {
      ...
      if (e.shiftKey && e.key === 'ArrowDown') {
        ...
        const now = Date.now();
        setCustomTimeRange(Math.round(mid - halfNewDur), Math.min(Math.round(mid + halfNewDur), now));
        return;
      }

      if (e.key === 't' && !e.ctrlKey && !e.metaKey && !e.altKey) {
        e.preventDefault();
        const trigger = document.querySelector('[data-testid="time-range-trigger"]') as HTMLButtonElement | null;
        trigger?.click();
        return;
      }
