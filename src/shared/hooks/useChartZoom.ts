...
export function useChartZoom(timestamps: (number | string)[]) {
  ...
  const handleDataZoom = useCallback(
    (params: any) => {
      ...
      const endIdx = Math.min(timestamps.length - 1, Math.ceil((batch.end ?? 100) / 100 * timestamps.length));

      const startTs = typeof timestamps[startIdx] === 'string'
        ? new Date(timestamps[startIdx]).getTime()
        : timestamps[startIdx] as number;
      const endTs = typeof timestamps[endIdx] === 'string'
